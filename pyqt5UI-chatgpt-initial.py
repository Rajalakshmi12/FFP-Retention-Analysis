...
class RetentionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.selected_sessions = 4  # default value
        ...
        for i in range(1, 11):
            ...
            btn.clicked.connect(lambda _, count=i: self.set_and_run(count))
        ...
        # Automatically calculate for default = 4
        self.set_and_run(4)

    def set_and_run(self, val):
        self.selected_sessions = val
        self.info_label.setText(f"You selected <b>{val}</b> sessions per month for retention calculation.")
    ...
    def calc_retention(self, dynamic_sessions):
        ...
        pct = round(len(retained) / len(eligible_ids) * 100, 2) if len(eligible_ids) > 0 else 0
        self.result_label.setText(
            f"<b>{len(retained)}</b> out of <b>{len(eligible_ids)}</b> eligible participants "
            f"had ≥<b>{dynamic_sessions}</b> sessions in the last two months "
            f"(<b>{pct}%</b>)."
        )
    ...
    def weekly_activity(self, category):
        ...
        eligible = self.df.groupby("Attendee ID")["Date"].min()
        eligible_ids = eligible[eligible < cutoff].index
        df_eligible = self.df[self.df["Attendee ID"].isin(eligible_ids)]
        ...
        act_text = " and ".join(top_activities)
...
# Update the code to make below modifications:
# 1. Remove spaces before result
# 2. Increase button width
# 3. Mark the selection button as green and let it stay green until they chose another value for recalculation